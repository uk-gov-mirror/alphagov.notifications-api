...
def test_success_should_log_and_call_statsd(mocker, notify_api, async_task):
    ...
    statsd.assert_called_with(f'celery.test-queue.{async_task.name}.success', 5.0)
    logger.assert_called_with(f'Celery task {async_task.name} (queue: test-queue) took 5.0000')
...
def test_success_queue_when_applied_synchronously(mocker, notify_api, celery_task):
    ...
    statsd.assert_called_with(f'celery.none.{celery_task.name}.success', 5.0)
    logger.assert_called_with(f'Celery task {celery_task.name} (queue: none) took 5.0000')
...
def test_failure_should_log_and_call_statsd(mocker, notify_api, async_task):
    ...
    statsd.assert_called_with(f'celery.test-queue.{async_task.name}.failure')
    logger.assert_called_with(f'Celery task {async_task.name} (queue: test-queue) failed')
...
def test_failure_queue_when_applied_synchronously(mocker, notify_api, celery_task):
    ...
    statsd.assert_called_with(f'celery.none.{celery_task.name}.failure')
    logger.assert_called_with(f'Celery task {celery_task.name} (queue: none) failed')
